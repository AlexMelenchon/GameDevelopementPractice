...
void j1Map::PropagateBFS()
{
	...
	if (frontier.Count > 0)
	{
		iPoint PopPoint = frontier.GetLast;
		frontier.Pop(frontier.GetLast);

		for (p2Queue_item<iPoint>* iterator = frontier.start; iterator != NULL; iterator = iterator->next)
		{

			if ((iterator->data.x == abs(PopPoint.x) || iterator->data.x == abs(PopPoint.x + 1) || iterator->data.x == abs(PopPoint.x - 1)) && (iterator->data.y == abs(PopPoint.y) || iterator->data.y == abs(PopPoint.y + 1) || iterator->data.y == abs(PopPoint.y - 1)))
