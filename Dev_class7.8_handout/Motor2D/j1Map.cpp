...
void j1Map::Path(int x, int y)
{
	...
	for (p2List_item<iPoint>* iterator = visited.start; iterator !=  NULL; iterator = iterator->next)
	{
		if (goal == iterator->data)
		{
			pathFind = true;
			break;
		}
	...
	if (frontier.Pop(curr))
	{
		...
		for (uint i = 0; i < 4; ++i)
		{
				...
				if (MovementCost(neighbors[i].x, neighbors[i].y) >= 0 )
				{
					...
					if (visited.find(neighbors[i]) == -1 || newCost < cost_so_far[neighbors[i].x][neighbors[i].y])
					{
